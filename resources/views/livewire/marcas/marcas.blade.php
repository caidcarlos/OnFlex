        ...
        <div class="mt-1 mx-auto">
            <div class="w-11/12 mx-auto">
                <button class="inline-flex items-center px-4 py-2 rounded-md bg-gray-700 font-bold text-sm text-white hover:text-green-400 active:bg-gray-700 focus:outline-none focus:border-gray-700 focus:ring focus:ring-gray-600 disabled:opacity-50 transition"
                    wire:click.prevent = "registrarMarca()">
                    Nueva Marca
                </button>
            </div>
            <div class="w-11/12 sm:mx-auto md:flex md:justify-between mt-2 mx-auto">
                ...
                @if($modalCreate)
                    @include('livewire.marcas.modalCreate')
                @endif
                @if($modalUpdate)
                    @include('livewire.marcas.modalUpdate')
                @endif
                @if($modalConfirm)
                    @include('livewire.marcas.modalConfirm')
                @endif
                    ...
                    <tbody>
                        @foreach ($marcas as $marca)
                            <tr class="hover:bg-gray-200 text-sm text-center">
                                <td>{{$marca->nombre}}</td>
                                <td>
                                    <div class="md:flex md:justify-center text-center my-2">
                                            <button class="w-full md:mr-1 sm:mr-0 md:mb-0 sm:mb-2 inline-flex items-center px-4 py-2 rounded-md bg-gray-700 font-bold text-center text-sm text-white hover:text-green-400 active:bg-gray-700 focus:outline-none focus:border-gray-700 focus:ring focus:ring-gray-600 disabled:opacity-50 transition"
                                                wire:click.prevent = "editar({{$marca->id}})">
                                                Editar
                                            </button>  
                                            <button class="w-full md:ml-1 sm:ml-0  md:mb-0 sm:mb-2 inline-flex items-center px-4 py-2 rounded-md bg-gray-700 font-bold text-center text-sm text-white hover:text-green-400 active:bg-gray-700 focus:outline-none focus:border-gray-700 focus:ring focus:ring-gray-600 disabled:opacity-50 transition"
                                                wire:click.prevent = "verStatus({{$marca->id}})">
